...
int ParseFile() {
	...
	// IMAGE_NT_HEADERS
	imageNTHeaders = (PIMAGE_NT_HEADERS)((DWORD)lpBase + dosHeader->e_lfanew);
	printf("\n******* NT HEADERS *******\n");
	...
	rawOffset = (DWORD)lpBase + importSection->PointerToRawData;
	...
	//pIED = (PIMAGE_EXPORT_DIRECTORY)(rawOffset + (imageNTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress - exportSection->VirtualAddress));
	pIED = (PIMAGE_EXPORT_DIRECTORY)((LPBYTE)((LPBYTE)lpBase + exportSection->PointerToRawData) + imageNTHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);

	PDWORD Address = (PDWORD)((LPBYTE)lpBase + pIED->AddressOfFunctions);
	PDWORD Name = (PDWORD)((LPBYTE)lpBase + pIED->AddressOfNames);

	PWORD Ordinal = (PWORD)((LPBYTE)lpBase + pIED->AddressOfNameOrdinals);

	for (int i = 0; i < pIED->AddressOfFunctions; i++)
	{
		printf("\t\t%s\n", (char*)lpBase + Name[i]);

		/*if (!strcmp(FunctionName, (char*)lpBase + Name[i]))
		{
